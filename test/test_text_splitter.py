...
@pytest.mark.parametrize(
    ...
    [
        ...
        (DOCUMENT_3, [511, 510, 506, 402], 0),
        (DOCUMENT_3, [511, 508, 511, 510, 413], 128)
    ]
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (DOCUMENT_3, [123, 124, 128, 118, 120, 124, 124, 125, 121, 121, 125, 125, 124, 122, 126, 67], 0),
        (DOCUMENT_3, [123, 123, 120, 126, 123, 125, 126, 128, 127, 122, 121, 125, 125, 123, 123, 125, 126, 24], 10)
    ]
)
def test_char_splits_with_paragraph_honor(doc, expected_lengths, stride, text_splitter):
    ...
    print([text_payload.meta["splitter"].chunk_length for text_payload in doc_splits])
    assert len(expected_lengths) == len(doc_splits)
    for text_payload, expected_length in zip(doc_splits, expected_lengths):
        assert "splitter" in text_payload.meta
        splitter_payload = text_payload.meta["splitter"]
