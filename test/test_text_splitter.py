...
def test_char_splits_without_paragraph_honor(
    doc, expected_lengths, stride, text_splitter
):
    doc_splits = text_splitter.preprocess_input(
        input_list=[TextPayload(processed_text=doc)],
        config=TextSplitterConfig(max_split_length=512, split_stride=stride),
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            DOCUMENT_2,
            [149, 108, 122, 172, 159, 133, 194, 100, 130, 270, 104, 155, 98],
            0,
        ),
        (
            DOCUMENT_2,
