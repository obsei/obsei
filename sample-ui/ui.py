...
pipeline_col, spinner_col, execute_col, download_python_col, download_yaml_col = st.beta_columns([2, 2, 1, 1, 1])
...
analyzer_icon = get_icon_name(None, configuration["analyzer"][selected_analyzer]["_icon_"])
...
render_config(src_config, source_col, configuration["source"][selected_source]["_help_"])
...
render_config(analyzer_config, analyzer_col, analyzer_type_config[selected_analyzer_type]["_help_"])
if len(analyzer_type_config[selected_analyzer_type]["analyzer"]) > 1:
    render_config(analyzer_type_config[selected_analyzer_type]["analyzer"], analyzer_col)
