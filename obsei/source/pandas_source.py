...
class PandasSourceConfig(BaseSourceConfig):
    ...
    def __init__(self, **data: Any):
        ...
        if not all([text_column in self.dataframe.columns for text_column in self.text_columns]):
            raise ValueError('Every `text_columns` should be present in `dataframe`')

        try:
            self.dataframe[self.text_columns] = self.dataframe[self.text_columns].astype('string')
...
class PandasSource(BaseSource):
    ...
    def lookup(self, config: PandasSourceConfig, **kwargs) -> List[TextPayload]:  # type: ignore[override]
        is_include_all_cols = config.include_columns is None
        df_to_records = config.dataframe.to_dict('records')
        source_responses: List[TextPayload] = [
            TextPayload(
                processed_text=config.separator.join([record.get(text_column) for text_column in config.text_columns]),
                meta=record if is_include_all_cols else {key: record[key] for key in config.include_columns},
