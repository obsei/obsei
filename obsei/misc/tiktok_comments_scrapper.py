...
class TiktokCommentsScrapper(BaseModel):
    ...
    async def fetch_tiktok_comments(self, until_datetime=None):
        try:
            command = "xvfb-run -a python3 libs/tiktok/process_tiktok.py " +  self.video_url + " " + str(self.max_comments) + " " + str(self.ms_token) + " " + str(until_datetime.strftime("%Y-%m-%d %H:%M:%S"))
